...
func CopyWithOptional(toValue interface{}, fromValue interface{}, opts ...Optional) error {
	o := defaultOption()
...
func copier(toValue interface{}, fromValue interface{}, opt Option) (err error) {
	...
	if from.Kind() == reflect.Slice || to.Kind() == reflect.Slice {
		...
		if to.Kind() == reflect.Slice && amount <= 0 && opt.InitNilSlice {
		...
		if source.IsValid() {
			...
			for _, field := range fromTypeFields {
				...
				if fromField := source.FieldByName(srcFieldName); fromField.IsValid() && !shouldIgnore(fromField, opt.IgnoreEmpty) {
					...
					if fromField.Kind() == reflect.Ptr && fromField.IsNil() && opt.InitNilPointer {
						fromField = reflect.New(reflect.TypeOf(fromField)).Elem()
					}
